...
class Collection:
    ...
    def _check(self, file) -> bool:
        ...
        for key, value in self.params.items():
            if isinstance(value, (int, float)) or key == "tspan":
                continue
    ...
    def _create(self):
        ...
        for i in range(self.n):
            ...
            z0 = params["z0"][i]
            Pz0 = params["Pz0"][i]
            tspan = params["tspan"][i]  # t0, tf, steps

            init_cond = [theta0, psi0, z0, Pz0]

            # Particle Creation
            p = Particle(species, mu, init_cond, tspan, R, a, q, Bfield, Efield, method=self.method)
